...
export function calculateVariablePay(role: RoleConfig, demand: number): number {
  ...
  const demandPosition = Math.min(Math.max(demand - role.demandMin, 0), demandRange);
  const variableRange = role.variableMax - role.variableMin;
  const variablePercent = role.variableMin + (demandPosition / demandRange) * variableRange;
  
  return (variablePercent / 100) * demand;
}

// Calculate team bonus for technical leaders
export function calculateTeamBonus(teamSize: number, avgPerformance: number): number {
...
export function calculateCompensation(employee: Employee): Compensation {
  ...
  if (employee.path === 'leadership') {
    if (employee.teamSize && ['tech_leader_1', 'tech_leader_2'].includes(employee.role)) {
