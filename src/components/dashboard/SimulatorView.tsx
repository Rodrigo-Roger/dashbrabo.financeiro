...
import { ROLES, CareerLevel, formatCurrency, calculateVariablePay } from "@/lib/data";
...
export function SimulatorView({ className }: SimulatorViewProps) {
    ...
    <div className={cn("space-y-6", className)}>
      <div className="rounded-xl bg-card p-6 shadow-card">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-foreground">Simulador de Remuneração</h3>
            <p className="text-sm text-muted-foreground">Simule diferentes cenários de performance</p>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <Label>Cargo</Label>
              <Select value={selectedRole} onValueChange={(v) => setSelectedRole(v as CareerLevel)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            {role.demandMin !== undefined && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-foreground">{formatCurrency(demand)}</span>
                </div>
                <Slider
                  value={[demand]}
            ...
            {role.quarterlyStay && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-foreground">{formatCurrency(quarterlyRevenue)}</span>
                </div>
                <Slider
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg border border-border p-4">
              ...
              <p className="text-3xl font-bold text-success">{formatCurrency(variablePay)}</p>
            ...
            {role.quarterlyStay && (
              <div className="rounded-lg bg-muted/50 p-4">
                <p className="text-sm text-muted-foreground mb-2">Status de Performance</p>
                {quarterlyRevenue >= (role.quarterlyPromotion || Infinity) ? (
                  <p className="font-semibold text-success">✓ Elegível para Promoção</p>
                ) : quarterlyRevenue >= role.quarterlyStay ? (
                  <p className="font-semibold text-foreground">✓ Meta de Permanência Atingida</p>
                ) : (
                  <p className="font-semibold text-danger">⚠ Abaixo da Meta de Permanência</p>
                )}
              </div>
            )}
          </div>
